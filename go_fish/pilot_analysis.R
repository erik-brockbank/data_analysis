...
get_eval_summary = function(eval_data) {
  eval_summary = eval_data %>%
    group_by(is_control, is_target_rule) %>%
...
get_time_summary = function(time_data) {
  time_summary = time_data %>%
    mutate(experiment_completion_time = expt_end_ts - expt_start_ts) %>%
    # NB: we only need subjID here but including it makes later group_by easier
    group_by(is_control, subjID) %>%
    summarize(mean_expt_time = mean(experiment_completion_time)) %>%
    group_by(is_control) %>%
    summarize(mean_task_time = mean(mean_expt_time))
...
plot_eval_results = function(eval_data) {
  eval_data %>%
    ggplot(aes(x = is_target_rule, y = mean_rating, 
               color = is_control, fill = is_control)) +
    geom_bar(stat = "identity", position = "dodge", width = 0.5, alpha = 0.5) +
...
plot_time_data = function(time_summary) {
  time_summary %>%
    ...
    labs(x = "", y = "Mean time on experiment") +
...
# Analysis: generation free response
generation_data = read_data(GENERATION_RESP_DATA)
# TODO how to code/present this?
