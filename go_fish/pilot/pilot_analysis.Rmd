...
## Time on Task
```{r completion_time_ttest, echo=FALSE, message=FALSE}
...
Indeed, the figure below suggests that explainers spent longer on the experiment overall, though this difference is not significant (*t* = `r round(t$statistic, 2)`, *p* = `r round(t$p.value, 2)`).

For future versions, we'll isolate the time spent on the explanation/description trials rather than the whole experiment. However, it may be worth putting in place additional checks to limit time on task or exploit behaviors that the control describers may be better at. 
